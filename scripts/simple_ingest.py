...
def parse_pdf(filepath: Path) -> dict:
    ...
    content = {"text": "", "tables": [], "metadata": {}, "pages": []}

    with pdfplumber.open(filepath) as pdf:
        content["metadata"] = {"num_pages": len(pdf.pages), "filename": filepath.name}
        ...
        for i, page in enumerate(pdf.pages):
            ...
            for table in tables:
                content["tables"].append({"page": i + 1, "data": table})

            content["pages"].append(
                {"page_num": i + 1, "text": page_text, "tables": tables}
            )
...
async def main():
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT doc_id, filename FROM documents ORDER BY created_at DESC"
        )
