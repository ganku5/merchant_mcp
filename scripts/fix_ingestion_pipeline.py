...
class FixedDocumentIngester:
    ...
    async def ingest_pdf_robust(
        ...
        if doc_id is None:
            doc_id = (
                path.stem.lower().replace(" ", "_").replace("-", "_").replace(".", "_")
            )
    ...
    def _try_pdftotext(self, filepath: str) -> str:
        ...
        result = subprocess.run(
            ["pdftotext", filepath, "-"], capture_output=True, text=True, timeout=30
        )
    ...
    async def _store_chunks_with_embeddings_safe(
        ...
        for i in range(0, len(chunks), self.EMBEDDING_BATCH_SIZE):
            ...
            total_batches = (
                len(chunks) + self.EMBEDDING_BATCH_SIZE - 1
            ) // self.EMBEDDING_BATCH_SIZE

            print(
                f"      Batch {batch_num}/{total_batches} (chunks {i + 1}-{min(i + len(batch), total)})..."
            )
...
async def main():
    ...
    pdfs = [
        (
            str(
                Path.home()
                / "Downloads/ibmb/[Axis] IBMB Bank Server API Specifications.pdf"
            ),
            "ibmb_axis_api_specs",
        ),
        (
            str(
                Path.home() / "Downloads/ibmb/IBMB Acquiring - Merchant Integration.pdf"
            ),
            "ibmb_acquiring_guide",
        ),
        (
            str(
                Path.home()
                / "Downloads/ibmb/IBMB BO_User Manual_PA Portal_v1.0 (2)-1.pdf"
            ),
            "ibmb_pa_portal_manual",
        ),
    ]
    ...
    async with database.pool.acquire() as conn:
        ...
        docs = await conn.fetch(
            "SELECT doc_id, total_chars FROM documents ORDER BY doc_id"
        )
        print(f"\nDocuments: {len(docs)}")
        for d in docs:
