...
PDF_DIR = str(Path.home() / "Downloads/ibmb")
...
async def main():
    ...
    pdfs = [
        (
            os.path.join(PDF_DIR, "IBMB Acquiring - Merchant Integration.pdf"),
            "ibmb_acquiring_guide",
        ),
        (
            os.path.join(PDF_DIR, "IBMB BO_User Manual_PA Portal_v1.0 (2)-1.pdf"),
    ...
    for doc_id, success, result in results:
        if success:
            print(
                f"✅ {doc_id}: {result['chunks']} chunks from {result['chars']:,} chars"
            )
        else:
    ...
    async with database.pool.acquire() as conn:
        ...
        for d in docs:
            status = (
                "✅"
                if d["embedded"] == d["total_chunks"] and d["total_chunks"] > 0
                else "⚠️"
            )
            print(
                f"  {status} {d['doc_id']}: {d['total_chunks']} chunks, {d['embedded']} embedded"
            )
