...
FIXTURES_DIR = "/home/ganesh/merchant_mcp/tests/fixtures/ground_truth"
    ...
    for ep in ibmb_endpoints:
        try:
            await database.insert_endpoint_spec(
                ...
                error_responses=[
                    {"error_code": "ERR001", "http_status": 400, "description": "Invalid request"},
                    {"error_code": "ERR002", "http_status": 401, "description": "Authentication failed"},
                ],
                spec_data=ep,
...
async def ingest_ibmb_csv_errors():
    ...
    csv_path = os.path.join(IBMB_DIR, "IBMB Error Codes with Description v 3 2.xlsx - IBMB to PA & Bank.csv")
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if any(x in error_code_upper for x in ['TIMEOUT', 'GATEWAY', 'NETWORK', 'SERVICE', 'UNAVAILABLE']):
                category = "retryable"
            elif any(x in error_code_upper for x in ['INVALID', 'MISSING', 'FORMAT', 'REQUIRED', 'NOT_FOUND']):
                category = "merchant_action"
            elif any(x in error_code_upper for x in ['DECLINED', 'REJECTED', 'BLOCKED', 'EXPIRED', 'UNAUTHORIZED']):
                category = "terminal"
            else:
...
async def ingest_pdf_document(pdf_path: str, doc_id: str):
    ...
    async with database.pool.acquire() as conn:
        ...
        await conn.execute("""
            ...
            VALUES ($1, $2, $3, $4, $5, $6)
        """, doc_id, fname, text[:100000], 0, len(text), 'pdf')
    ...
    embedded = await database.pool.fetchval(
        "SELECT COUNT(*) FROM text_chunks WHERE doc_id = $1 AND embedding IS NOT NULL", doc_id
    )
    ...
    flows = [
        {
            ...
            "steps": [
                {"step_number": 1, "name": "Create Order", "description": "Call orders.create", "endpoint_id": "orders.create"},
                {"step_number": 2, "name": "Redirect Customer", "description": "Send to payment page"},
                {"step_number": 3, "name": "Handle Webhook", "description": "Process order.charged webhook"},
            ],
        ...
        },
        {
            ...
            "steps": [
                {"step_number": 1, "name": "Check Order Status", "description": "Verify CHARGED status"},
                {"step_number": 2, "name": "Create Refund", "description": "Call refund.create"},
            ],
        ...
        },
        {
            ...
            "steps": [
                {"step_number": 1, "name": "Fetch Transaction", "description": "Call ibmb.axis.sdk.fetch"},
                {"step_number": 2, "name": "Authenticate", "description": "Call ibmb.axis.sdk.auth"},
                {"step_number": 3, "name": "Process Payment", "description": "Call ibmb.axis.sdk.pay"},
            ],
...
async def create_webhook_events():
    ...
    events = [
        ...
        },
        {
            ...
            "payload_schema": {"order_id": "string", "status": "CHARGED", "payment": {}},
        },
        {
            "event_type": "order.failed",
            "description": "Triggered when payment fails",
            "payload_schema": {"order_id": "string", "status": "FAILED", "error_code": "string"},
        },
        {
            "event_type": "refund.processed",
            "description": "Triggered when refund is processed",
            "payload_schema": {"refund_id": "string", "order_id": "string", "status": "SUCCESS"},
        },
    ]
    ...
    async with database.pool.acquire() as conn:
        for event in events:
            try:
                ...
                """, event["event_type"], event["description"], json.dumps(event["payload_schema"]))
                count += 1
...
async def create_test_scenarios():
    ...
    scenarios = [
        ...
        },
        {
            ...
            "input_data": {"order_id": "existing_order", "unique_request_id": "refund_001"},
            "expected_http_status": 200,
        },
    ]
    ...
    async with database.pool.acquire() as conn:
        for sc in scenarios:
            try:
                ...
                """, sc["scenario_id"], sc["flow_type"], sc["name"], sc["description"],
                     json.dumps(sc["input_data"]), sc["expected_http_status"])
                count += 1
...
async def show_summary():
    ...
    async with database.pool.acquire() as conn:
        stats = {
            ...
            "Chunks with embeddings": await conn.fetchval("SELECT COUNT(*) FROM text_chunks WHERE embedding IS NOT NULL"),
            ...
            "Integration flows": await conn.fetchval("SELECT COUNT(*) FROM integration_flows"),
            "Webhook events": await conn.fetchval("SELECT COUNT(*) FROM webhook_events"),
            "Test scenarios": await conn.fetchval("SELECT COUNT(*) FROM test_scenarios"),
        }
        ...
        # Show endpoints
        print("\n🔌 Endpoints:")
        eps = await conn.fetch("SELECT endpoint_id, method, path FROM endpoint_specs ORDER BY endpoint_id")
        for e in eps:
            gt = "GT" if e['endpoint_id'].startswith(('orders.', 'order.', 'refund.')) else "IBMB"
            print(f"  [{gt}] {e['method']:<6} {e['path']}")


async def main():
    parser = argparse.ArgumentParser(description="Unified ingestion for Merchant MCP")
    parser.add_argument("--all", action="store_true", help="Ingest everything (default)")
    ...
    parser.add_argument("--flows", action="store_true", help="Ingest flows/webhooks/tests")
    ...
    if args.clear:
        ...
        async with database.pool.acquire() as conn:
            ...
            await conn.execute("UPDATE endpoint_specs SET source_doc_id = NULL WHERE source_doc_id IS NOT NULL")
            await conn.execute("UPDATE error_codes SET source_doc_id = NULL WHERE source_doc_id IS NOT NULL")
            await conn.execute("UPDATE integration_flows SET source_doc_id = NULL WHERE source_doc_id IS NOT NULL")
            await conn.execute("DELETE FROM documents")
            await conn.execute("DELETE FROM endpoint_specs WHERE is_ground_truth = false")
            await conn.execute("DELETE FROM error_codes WHERE source_doc_id = 'ibmb_error_codes_csv'")
