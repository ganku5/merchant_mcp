...
async def main():
    ...
    endpoints = {
        "orders.create": {
            ...
            "desc": "Create payment order - FIRST API CALL",
            "fields": [
                {"name": "order_id", "type": "string", "req": True},
                {
                    "name": "amount",
                    "type": "integer",
                    "req": True,
                    "desc": "PAISE - multiply rupees by 100",
                },
                {"name": "currency", "type": "string", "req": True, "enum": ["INR"]},
                {"name": "customer_email", "type": "string", "req": True},
                {"name": "return_url", "type": "string", "req": True},
            ],
            "errors": ["ORDER_ID_EXISTS", "INVALID_AMOUNT", "INVALID_EMAIL"],
        },
        "payment.status": {
            "method": "GET",
            "path": "/v1/orders/{order_id}",
            "desc": "Check status - USE FOR POLLING",
            "fields": [{"name": "order_id", "in": "path", "req": True}],
            "poll": {"interval": 5, "max": 300},
        },
        "refunds.create": {
            "method": "POST",
            "path": "/v1/refunds",
            "desc": "Refund CHARGED order",
            "fields": [
                {"name": "order_id", "req": True},
                {"name": "amount", "req": False, "desc": "Omit for full refund"},
                {"name": "unique_request_id", "req": True, "desc": "Idempotency key"},
            ],
        },
    }
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "CREATE TABLE endpoint_specs (endpoint_id TEXT PRIMARY KEY, method TEXT, path TEXT, description TEXT, request_schema JSONB, response_schema JSONB, spec_data JSONB)"
        )
        await conn.execute(
            "CREATE TABLE error_codes (error_code TEXT PRIMARY KEY, http_status INTEGER, category TEXT, message TEXT, description TEXT, retry_data JSONB, error_data JSONB)"
        )

        for ep_id, data in endpoints.items():
            await conn.execute(
                "INSERT INTO endpoint_specs VALUES ($1, $2, $3, $4, $5, $6, $7)",
                ep_id,
                data["method"],
                data["path"],
                data["desc"],
                json.dumps({"fields": data["fields"]}),
                json.dumps({}),
                json.dumps(data),
            )
            print(f"  ✓ {ep_id}")
        ...
        for err in error_codes:
            await conn.execute(
                "INSERT INTO error_codes VALUES ($1, $2, $3, $4, $5, $6, $7)",
                err["code"],
                err["status"],
                err["cat"],
                err["msg"],
                err["msg"],
                json.dumps(err.get("retry", {})),
                json.dumps(err),
            )
    ...
    flows = [
        {
            ...
            "use_case": "payment",
            "steps": [
                {"n": 1, "name": "Create Order", "ep": "orders.create"},
                {"n": 2, "name": "Redirect to Payment"},
                {"n": 3, "name": "Handle Return"},
                {"n": 4, "name": "Verify Status", "ep": "payment.status"},
            ],
        },
        {
            "id": "refund_full",
            ...
            "use_case": "refund",
            "steps": [
                {"n": 1, "name": "Check Status", "ep": "payment.status"},
                {"n": 2, "name": "Create Refund", "ep": "refunds.create"},
            ],
        },
    ]
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "CREATE TABLE integration_flows (flow_id TEXT PRIMARY KEY, name TEXT, use_case TEXT, steps JSONB, flow_data JSONB)"
        )
        for f in flows:
            await conn.execute(
                "INSERT INTO integration_flows VALUES ($1, $2, $3, $4, $5)",
                f["id"],
                f["name"],
                f["use_case"],
                json.dumps(f["steps"]),
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "CREATE TABLE test_scenarios (scenario_id TEXT PRIMARY KEY, flow_type TEXT, name TEXT, expected_status TEXT)"
        )
        for tc in test_cases:
            await conn.execute(
                "INSERT INTO test_scenarios VALUES ($1, $2, $3, $4)",
                tc["id"],
                tc["flow"],
                tc["name"],
                tc["expected"],
    ...
    known_issues = [
        {
            ...
            "resolution": "Use raw body bytes, not parsed JSON",
            "cat": "webhook",
        },
        {
            "id": "dup_order",
            "pattern": "ORDER_ID_EXISTS",
            "resolution": "Generate new unique ID per attempt",
            "cat": "payment",
        },
        {
            "id": "amt_format",
            "pattern": "INVALID_AMOUNT",
            "resolution": "Send paise (rupees * 100)",
            "cat": "payment",
        },
    ]
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "CREATE TABLE known_issues (issue_id TEXT PRIMARY KEY, pattern TEXT, resolution TEXT, category TEXT)"
        )
        for ki in known_issues:
            await conn.execute(
                "INSERT INTO known_issues VALUES ($1, $2, $3, $4)",
                ki["id"],
                ki["pattern"],
                ki["resolution"],
                ki["cat"],
    ...
    production_config = {
        "rate_limits": {
            "read_only": {"per_min": 100, "burst": 200},
            "sandbox": {"per_min": 20, "burst": 40},
        },
