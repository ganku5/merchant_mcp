...
async def extract_entities(
    text: str, schema: Dict[str, Any], entity_type: str
) -> List[Dict]:
...
async def ingest_single_pdf(pool, filepath: Path) -> Dict:
    ...
    async with pool.acquire() as conn:
        for ep in endpoints:
            ep_id = (
                ep.get("endpoint_id")
                or f"ep_{hashlib.md5(ep.get('path', '').encode()).hexdigest()[:8]}"
            )
            await conn.execute(
                """
    ...
    async with pool.acquire() as conn:
        for flow in flows:
            flow_id = (
                flow.get("flow_id")
                or f"flow_{hashlib.md5(flow.get('name', '').encode()).hexdigest()[:8]}"
            )
            await conn.execute(
                """
...
async def main():
    ...
    # Find PDFs
    ibmb_folder = Path.home() / "Downloads/ibmb"
    ...
    total_chunks = sum(
        r.get("chunks", 0) for r in results if r.get("status") == "success"
    )
    total_endpoints = sum(
        r.get("endpoints", 0) for r in results if r.get("status") == "success"
    )
    total_errors = sum(
        r.get("errors", 0) for r in results if r.get("status") == "success"
    )
    total_flows = sum(
        r.get("flows", 0) for r in results if r.get("status") == "success"
    )
