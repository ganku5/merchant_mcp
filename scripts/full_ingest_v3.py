    ...
    try:
        response = await llm_client.chat(
            [
                {
                    "role": "system",
                    "content": "You extract structured data from API docs. Return only valid JSON.",
                },
...
async def process_pdf(pool, filepath: Path) -> dict:
    ...
    async with pool.acquire() as conn:
        for ep in entities["endpoints"]:
            ep_id = (
                ep.get("endpoint_id")
                or f"ep_{hashlib.md5(ep.get('path', '').encode()).hexdigest()[:6]}"
            )
            await conn.execute(
                """
        ...
        for flow in entities["flows"]:
            fid = (
                flow.get("flow_id")
                or f"flow_{hashlib.md5(flow.get('name', '').encode()).hexdigest()[:6]}"
            )
            await conn.execute(
                """
    ...
    print(
        f"   ✓ {len(entities['endpoints'])} EPs, {len(entities['error_codes'])} ERRs, {len(entities['flows'])} FLWs"
    )
...
async def main():
    ...
    pdfs = sorted((Path.home() / "Downloads/ibmb").glob("*.pdf"))
    print(f"\nFound {len(pdfs)} PDFs\n")
    ...
    print(
        f"Extracted: {total_eps} endpoints, {total_errs} error codes, {total_flows} flows"
    )
