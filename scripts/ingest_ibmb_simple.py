...
def categorize_error(error_code: str) -> str:
    ...
    if any(x in error_code for x in ['TIMEOUT', 'GATEWAY', 'NETWORK', 'SERVICE', 'UNAVAILABLE']):
        return "retryable"
    elif any(x in error_code for x in ['INVALID', 'MISSING', 'FORMAT', 'REQUIRED', 'NOT_FOUND']):
        return "merchant_action"
    elif any(x in error_code for x in ['DECLINED', 'REJECTED', 'BLOCKED', 'EXPIRED', 'UNAUTHORIZED']):
        return "terminal"
    else:
        return "system_error"
    ...
    endpoints = [
        {
            ...
            "error_responses": [
                {"error_code": "IBMB001", "http_status": 400, "description": "Request json object is not in standard format"},
                {"error_code": "IBMB002", "http_status": 400, "description": "Request is invalid"},
                {"error_code": "IBMB003", "http_status": 400, "description": "Reference ID is not sent"},
            ],
            "spec_data": {},
        ...
        },
        {
            ...
            "auth_type": "api_key",
            "request_schema": {"fields": [{"field_name": "transaction_id", "json_path": "$.path.transaction_id", "field_type": "string", "required": True}]},
            "response_schema": {"fields": [{"field_name": "status", "json_path": "$.status", "field_type": "string", "required": True}]},
            "error_responses": [{"error_code": "IBMB004", "http_status": 404, "description": "Request is not found"}],
            "spec_data": {},
            "is_ground_truth": False,
        },
    ]
