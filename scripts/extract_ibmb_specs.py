...
def parse_parameter_table(text):
    ...
    for line in lines:
        ...
        match = re.match(r'^(\d+)\s+(\S+)\s+(.+?)\s+(Alphanumeric|Numeric|Boolean|Array|Object|ENUM)\s+(M|O)\s*(.*)$', line)
...
async def extract_all_apis():
    ...
    async with database.pool.acquire() as conn:
        # Get PDF content
        doc = await conn.fetchrow("SELECT content FROM documents WHERE doc_id = 'ibmb_axis_api_specs'")
        ...
        apis = [
            ...
            },
            {
                ...
                'markers': ['Endpoint URL: {{host}}/api/sdk/v1/status', '3. Web Redirection'],
            ...
            },
            {
                ...
                'markers': ['Endpoint URL: {{host}}/api/web/v1/status', '4. API Error Codes'],
        ...
        for api in apis:
            ...
            section = extract_api_section(content, api['markers'][0], api['markers'][1:])
            ...
            api_spec = {
                ...
                'error_responses': [
                    {'error_code': 'ERR001', 'http_status': 400, 'description': 'Invalid request parameters'},
                    {'error_code': 'ERR002', 'http_status': 401, 'description': 'Authentication failed'},
                    {'error_code': 'ERR003', 'http_status': 500, 'description': 'Internal server error'}
                ],
                'spec_data': {
        ...
        for e in eps:
            ...
            print(f"    Request: {e['req_fields']} fields, Response: {e['resp_fields']} fields")
