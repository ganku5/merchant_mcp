# Lite Execute Mandate API Integration Guide

Source endpoint: `POST /api/{apiVersion}/merchants/mandates/liteExecute`

## Overview

Lite Execute Mandate is a server-to-server API used to execute an existing payer-side UPI Lite mandate. This is the primary public route for the lite execute flow. The older `/merchants/mandates/execute` route is still wired to the same handler as a backward-compatible alias.

The merchant calls this API after a mandate has already been created, approved, and linked to the merchant customer. Newton validates the merchant, customer, stored mandate, mandate amount/date/status rules, duplicate transaction id, and Lite mandate execution conditions. Newton then creates a `PAY` transaction and sends `ReqPay` to NPCI.
...
- Executing a customer-approved UPI Lite mandate from the merchant backend.
...
1. Merchant creates and approves a Lite mandate through the appropriate mandate creation flow.
...
4. Merchant builds the decrypted business payload, signs/encrypts it using the Newton S2S process, and calls `liteExecute`.
...
7. Newton creates a `PAY` transaction and sends a mandate `ReqPay` to NPCI, unless Sherlock/risk checks produce a terminal result before the NPCI call.
...
- `merchantRequestId`: Merchant-generated reference for this execute attempt. It is stored on the transaction and echoed in the payload.
...
```http
POST /api/{apiVersion}/merchants/mandates/liteExecute
```

The same request and response types are also exposed at:

```http
POST /api/{apiVersion}/merchants/mandates/execute
```

`/liteExecute` is the primary route. `/execute` is a backward-compatible alias and is mapped to the same handler, transformer, validator, product logic, and response mapping. There is no request or response body difference between the two paths in the current code path.
...
| `x-api-version` | Onboarding dependent | Some S2S integrations also send this header. When used, keep it consistent with the path version and onboarding instructions. |
...
| `merchantCustomerId` | string | Yes | No default. | 1 to 256 characters. Allowed characters: letters, numbers, `.`, `_`, `+`, `/`, `=`, and `-`; first character must be alphanumeric, `+`, `/`, or `=`. Must resolve to a merchant customer for the authenticated merchant. | Merchant's customer identifier linked to the mandate. Used during middleware lookup and verified against the stored mandate. |
...
- The public S2S request does not contain a Lite account object. The S2S transformer sets `_liteRecord = Nothing` before calling the shared core route. If a stored mandate is marked as Lite auto-topup and the product logic requires a Lite record, the request can fail Lite-specific validation; use the onboarded SDK auto-topup execute path where Newton has instructed you to do so.
- `upiRequestId` is the duplicate guard. Reusing it after Newton has created a transaction returns `DUPLICATE_REQUEST`.
- There are no nested request objects for this endpoint. `udfParameters`, when used, is a string containing a JSON object.

## Response

The examples show the decrypted business response. The actual transport body is the Newton `EncResponse` envelope. `flowWithTrace` also returns trace headers `x-requestid` and `x-sessionid`; response signing/encryption depends on the merchant's configured response strategy.
...
- Route type for `/liteExecute` and backward-compatible `/execute` alias: [src/Newton/App/Routes/Core.hs](../../src/Newton/App/Routes/Core.hs:540)
- Both route table entries mapped to the same handler: [src/Newton/App/Server.hs](../../src/Newton/App/Server.hs:299)
...
- S2S transformer and core request/response mapping, including `_liteRecord = Nothing`: [src/Newton/Services/Transformer/ServerToServer/Core.hs](../../src/Newton/Services/Transformer/ServerToServer/Core.hs:361), [src/Newton/Services/Transformer/ServerToServer/Helper.hs](../../src/Newton/Services/Transformer/ServerToServer/Helper.hs:1209)
- SDK-only Lite auto-topup core request mapping with `_liteRecord = Just liteRecord`: [src/Newton/Utils/Transformers/Transformer9.hs](../../src/Newton/Utils/Transformers/Transformer9.hs:2926)
...
- Shared validation rules: [src/Newton/Validation/Common.hs](../../src/Newton/Validation/Common.hs:246), [src/Newton/Validation/Common.hs](../../src/Newton/Validation/Common.hs:292), [src/Newton/Validation/Common.hs](../../src/Newton/Validation/Common.hs:351), [src/Newton/Validation/Common.hs](../../src/Newton/Validation/Common.hs:575)
...
- NPCI async call and pay response handling: [src/Newton/External/NPCI/Flow.hs](../../src/Newton/External/NPCI/Flow.hs:47), [src/Newton/Product/TransactionV2Helper.hs](../../src/Newton/Product/TransactionV2Helper.hs:467)
- Shared response/error constants: [src/Newton/Constants/APIErrorCode.hs](../../src/Newton/Constants/APIErrorCode.hs:43), [src/Newton/Constants/APIErrorCode.hs](../../src/Newton/Constants/APIErrorCode.hs:106), [src/Newton/Constants/APIErrorCode.hs](../../src/Newton/Constants/APIErrorCode.hs:581), [src/Newton/Constants/APIErrorCode.hs](../../src/Newton/Constants/APIErrorCode.hs:1149)
