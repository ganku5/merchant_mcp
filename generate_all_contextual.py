...
async def process_document(generator: ContextualEmbeddingGenerator, doc_id: str, filename: str):
...
async def main():
    ...
    for r in results:
        ...
        if r.get('success'):
            print(f"{status} {r['doc_id']}: {r['processed']} chunks, {r['generated']} Q&A pairs")
        else:
