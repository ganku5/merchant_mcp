...
IBMB_DOCS_DIR = str(Path.home() / "Downloads/ibmb")
...
def parse_api_specifications(text):
    ...
    api_sections = re.split(
        r"\n(?=(?:API Name|API Endpoint|Resource|Service)\s*:)",
        text,
        flags=re.IGNORECASE,
    )
...
def parse_api_section(section_text):
    ...
    for line in lines[1:]:
        ...
        if any(
            keyword in lower for keyword in ["description:", "overview:", "purpose:"]
        ):
        ...
        if any(
            k in lower for k in ["request header", "input header", "header (request)"]
        ):
        ...
        if any(
            k in lower
            for k in ["response header", "output header", "header (response)"]
        ):
        ...
        if any(
            k in lower for k in ["error response", "error code", "failure response"]
        ):
        ...
        if current_section == "request_headers" and line.startswith("-") or ":" in line:
            ...
            if field:
                target = (
                    api["request_fields"]
                    if current_section == "request_fields"
                    else api["response_fields"]
                )
                target.append(field)
    ...
    if not api["endpoint_id"] and api["path"]:
        ...
        api["endpoint_id"] = "ibmb." + ".".join(
            path_parts[-2:] if len(path_parts) >= 2 else path_parts
        )
...
def parse_field_line(line, field_stack):
    ...
    full_name = (
        f"{field['parent_path']}.{field['field_name']}"
    ...
    with open(error_codes_file, "r") as f:
        ...
        for line in f:
            ...
            if "," in line and not line.startswith("//"):
                ...
                error_codes.append(
                    {"error_code": code, "http_status": 400, "description": desc}
                )
...
def create_common_structure():
    ...
    head_fields = [
        {
            ...
            "description": "Message version (e.g., '1.0')",
        },
        {
            "field_name": "ts",
            "field_type": "string",
        ...
        },
        {
            ...
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Unique message ID (max 35 chars)",
        },
        {
            "field_name": "orgId",
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Organization ID (alphanumeric, 5 chars)",
        },
        {
            "field_name": "prodType",
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Product type - must be 'IBMB'",
        },
        {
            "field_name": "orgType",
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Organization type - 'PA' or 'BANK'",
        },
    ]
    ...
    txn_fields = [
        {
            ...
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Transaction ID (20 chars alphanumeric)",
        },
        {
            "field_name": "refId",
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Reference ID (20 chars alphanumeric)",
        },
        {
            "field_name": "initiationMode",
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Initiation mode - SDK or REDIRECTION",
        },
        {
            "field_name": "txnTs",
            "field_type": "string",
        ...
        },
        {
            ...
            "field_type": "string",
            "requirement": "mandatory",
            "description": "Transaction amount (numeric, max 3 decimal places)",
        },
        {
            "field_name": "cur",
            "field_type": "string",
        ...
        },
        {
            ...
            "field_type": "string",
            "requirement": "optional",
            "description": "Transaction note (max 255 chars)",
        },
        {
            "field_name": "expiry",
            "field_type": "integer",
    ...
    payer_fields = [
        {
            ...
            "condition_description": "Required for UPI payments",
    ...
    return {"head": head_fields, "transaction": txn_fields, "payer": payer_fields}
...
def main():
    ...
    api_spec_pdf = os.path.join(
        IBMB_DOCS_DIR, "[Axis] IBMB Bank Server API Specifications.pdf"
    )
    error_codes_csv = os.path.join(
        IBMB_DOCS_DIR,
