...
async def main():
    ...
    success, msg = await test_tool(
        "get_integration_guide", get_integration_guide, "payment", "python"
    )
    ...
    # 5. generate_payload
    success, msg = await test_tool(
        "generate_payload", generate_payload, "orders.create", {}, False
    )
    ...
    # 6. get_code_example
    success, msg = await test_tool(
        "get_code_example", get_code_example, "orders.create", "python"
    )
    ...
    success, msg = await test_tool(
        "get_webhook_handler", get_webhook_handler, "order.charged", "python"
    )
    ...
    success, msg = await test_tool(
        "validate_payload", validate_payload, "orders.create", test_payload
    )
    ...
    success, msg = await test_tool(
        "test_sandbox", test_sandbox, "orders.create", test_payload, None
    )
    ...
    # 11. get_test_cases
    success, msg = await test_tool(
        "get_test_cases", get_test_cases, "payment", "essential"
    )
    ...
    success, msg = await test_tool(
        "check_integration", check_integration, "pre_production"
    )
    ...
    test_headers = {
        ...
        "x-juspay-signature": "test_sig",
    }
    test_body = '{"event": "order.charged", "order_id": "test_001"}'
    success, msg = await test_tool(
        "diagnose_webhook", diagnose_webhook, test_headers, test_body
    )
    ...
    # 14. lookup_error_map
    success, msg = await test_tool(
        "lookup_error_map", lookup_error_map, "ORDER_ID_EXISTS"
    )
    ...
    success, msg = await test_tool(
        "search_known_issues", search_known_issues, "webhook not received"
    )
